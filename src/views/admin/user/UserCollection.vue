...
import {ArticleQueryCategory, ArticleUpdate, TagArticle} from "@/api/imagetext";
...
import {UserCollection, UserRemoveCollection} from "@/api/user";
//固定参数--------------------------------------------------------------------------
//表头
...
const loadingData = async () => {
  let res = await UserCollection(page, tag.value, category.value)
  if (res.code) {
...
const deleteData = async (ids: string | string[]) => {
  ...
  let res = await UserRemoveCollection(id_list)
  if (res.code) {
    ElMessage.error(res.msg)
    return
  }
